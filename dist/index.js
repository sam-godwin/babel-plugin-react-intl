...
function evaluateMessageDescriptor(descriptorPath, isJSXSource = false, filename, idInterpolationPattern = "[contenthash:5]", overrideIdFn) {
    ...
    const partnerVariations = getMessageDescriptorValue(descriptorPath.partnerVariations);
    if (overrideIdFn) {
...
function storeMessage({ id, description, defaultMessage, partners, partnerVariations, }, path, { extractSourceLocation }, filename, messages) {
...
function isFormatMessageDestructuring(scope) {
    ...
    if (core_1.types.isFunctionDeclaration(block) &&
        ...
        return block.params[0].properties.find((value) => value.key.name === "intl");
    }
    return false;
}
...
exports.default = (0, helper_plugin_utils_1.declare)((api, options) => {
    ...
    return {
        ...
        visitor: {
            ...
            JSXOpeningElement(path, { opts, file: { opts: { filename }, }, }) {
                const { moduleSourceName = "react-intl", additionalComponentNames = [], removeDefaultMessage, idInterpolationPattern, overrideIdFn, ast, } = opts;
                ...
                if (name.isJSXIdentifier() &&
                    ...
                    if (descriptorPath.id || descriptorPath.defaultMessage) {
                        ...
                        const descriptor = evaluateMessageDescriptor(descriptorPath, true, filename, idInterpolationPattern, overrideIdFn);
                        storeMessage(descriptor, path, opts, filename, this.ReactIntlMessages);
                        let idAttr;
                        let descriptionAttr;
                        let defaultMessageAttr;
                        let partnersAttr;
                        let partnerVariationsAttr;
                        ...
                        if (!removeDefaultMessage &&
                            ...
                            defaultMessageAttr.get("value")
                                .get("expression")
                                .replaceWithSourceString(JSON.stringify((0, intl_messageformat_parser_1.parse)(descriptor.defaultMessage)));
                        }
            ...
            CallExpression(path, { opts, file: { opts: { filename }, }, }) {
                const { ReactIntlMessages: messages } = this;
                const { moduleSourceName = "react-intl", overrideIdFn, idInterpolationPattern, removeDefaultMessage, extractFromFormatMessageCall, ast, } = opts;
                ...
                function processMessageObject(messageDescriptor) {
                    ...
                    const descriptor = evaluateMessageDescriptor(descriptorPath, false, filename, idInterpolationPattern, overrideIdFn);
                    storeMessage(descriptor, messageDescriptor, opts, filename, messages);
                    // Remove description since it's not used at runtime.
                    messageDescriptor.replaceWithSourceString(JSON.stringify({
                        id: descriptor.id,
                        ...(!removeDefaultMessage && descriptor.defaultMessage
                            ? {
                                defaultMessage: ast
                                    ? (0, intl_messageformat_parser_1.parse)(descriptor.defaultMessage)
                                    : descriptor.defaultMessage,
                            }
                            : {}),
                    }));
